<x-admin-layout title="Edit Produk Karebla">
    @php
        $specsArray = [];
        if (!empty($produk->specs)) {
            foreach($produk->specs as $k => $v) {
                $specsArray[] = ['key' => $k, 'value' => $v];
            }
        }
    @endphp
    <div class="py-12" x-data="specManager({{ json_encode($specsArray) }})">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            
            <div class="flex items-center justify-between">
                <h2 class="text-2xl font-bold text-gray-800">Edit Produk Karebla</h2>
                ...
                <div class="p-6 md:p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $produk->name) }}" required class="w-full rounded-lg border-gray-300 focus:border-[#D4AF37] focus:ring focus:ring-[#D4AF37] focus:ring-opacity-50 text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Koleksi (Opsional)</label>
                            <input type="text" name="collection" value="{{ old('collection', $produk->collection) }}" placeholder="Misal: Karebla Premium Collection" class="w-full rounded-lg border-gray-300 focus:border-[#D4AF37] focus:ring focus:ring-[#D4AF37] focus:ring-opacity-50 text-sm">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="coin_price" value="{{ old('coin_price', $produk->coin_price) }}" min="1" required class="w-full pl-9 rounded-lg border-gray-300 focus:border-[#D4AF37] focus:ring focus:ring-[#D4AF37] focus:ring-opacity-50 text-sm">
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Stok</label>
                            <input type="number" name="stock" value="{{ old('stock', $produk->stock) }}" min="0" required class="w-full rounded-lg border-gray-300 focus:border-[#D4AF37] focus:ring focus:ring-[#D4AF37] focus:ring-opacity-50 text-sm">
                        </div>
                        <div>
                            ...
                            <select name="badge" class="w-full rounded-lg border-gray-300 focus:border-[#D4AF37] focus:ring focus:ring-[#D4AF37] focus:ring-opacity-50 text-sm">
                                ...
                                <option value="eksklusif" {{ old('badge', $produk->badge) == 'eksklusif' ? 'selected' : '' }}>Eksklusif ✨</option>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" required class="w-full rounded-lg border-gray-300 focus:border-[#D4AF37] focus:ring focus:ring-[#D4AF37] focus:ring-opacity-50 text-sm">{{ old('description', $produk->description) }}</textarea>
                    </div>
                    ...
                    <div>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $produk->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-[#0F2942] focus:ring-[#D4AF37]">
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('specManager', (initialSpecs) => ({
                specs: initialSpecs.length ? initialSpecs : [],
                addSpec() {
